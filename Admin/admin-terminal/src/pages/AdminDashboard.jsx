...
const UserManagement = () => {
  ...
  const fetchUsers = async () => {
    try {
      // 2. Simple call, no manual headers needed
  ...
  const handleResendFile = async (projectId, file) => {
    ...
    try {
      ...
      // 3. Instance handles BaseURL, Auth, and path
      ...
      if (data.success) {
        ...
        fetchUsers(); // Refresh the list to show the "completed" checkmark
      }
    } catch (err) {
